...
const Top = () => {
  ...
  const [tabsList, setTabsList] = useState([
    { tab: <HomeOutlined style={{ color: "#667085" }} />, key: "home" },
  ]);
  ...
  const onEdit = (targetKey, action) => {
    if (action === "add") {
      ...
      if (targetKey === activeKey) {
        const newActiveKey =
          newTabsList.length > 0
            ? newTabsList[newTabsList.length - 1].key
            : "home";
  ...
  return (
    <header
      ...
      style={{ marginLeft: "80px", height: "70px" }}
    >
      ...
      >
        {tabsList.map((tabInfo) => (
          ...
          >
            {/* <div>
              This is content of {tabInfo.key === "home" ? "Home" : tabInfo.tab}
            </div> */}
          </Tabs.TabPane>
        ))}
      </Tabs>
